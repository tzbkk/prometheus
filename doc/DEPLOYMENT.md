...
│   │   ├── commands.py           # CommandParser + Dispatcher（纯逻辑层）
│   │   ├── shell.py              # prompt_toolkit 交互 shell + 状态栏 + 日志 tail
│   │   └── __main__.py           # 入口：装配 pm + api + monitor 线程 + shell
│   ├── tui/                      # textual 控制台
│   │   ├── app.py                # 主应用
...
```bash
bash scripts/start_launcher.sh
# launcher 启动 API + 交互 shell（不自动启动 QQ/TUI/Viewer）
# shell 子命令：start/stop/restart qq|tui|viewer, status, logs, config, health, tail, help, quit
# 底部状态栏实时显示三进程状态
# quit 退出（会杀 QQ + TUI + Viewer）
```
