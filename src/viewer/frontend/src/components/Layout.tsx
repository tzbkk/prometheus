 ...
 * output. Light theme only (white background, gray text); dark mode is
...
function StatsFooter() {
  ...
  return (
    <footer className="border-t border-gray-100 py-3">
      <p className="mx-auto max-w-5xl px-4 text-center text-xs text-gray-400">
        {data
          ? `帖子 ${data.feeds.toLocaleString()} · 评论 ${data.comments.toLocaleString()} · 媒体 ${data.media.toLocaleString()}`
          : '\u00a0'}
      </p>
    </footer>
  )
