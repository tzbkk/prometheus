...
def _ensure_column(conn: sqlite3.Connection, table: str, column: str, col_type: str) -> None:
    ...
    no ALTER fires. On DBs where the column is absent it is added with NULL
    for existing rows; the indexer's rebuild populates it afterwards.
    """
...
def init_db(db_path: str) -> sqlite3.Connection:
    ...
    For existing databases missing the guild_id columns,
    they are added in-place via ALTER TABLE (data preserved, NULL for
    existing rows).
    """
    ...
    # comment_media is fully rebuilt from the entity tree on every indexer
    # run, so drop+recreate (when an FK-bearing variant is found) is safe
    # and data-loss-free.
    ...
    _ensure_column(conn, "feeds", "content_text", "TEXT")
    _ensure_column(conn, "comments", "guild_id", "TEXT")
    _ensure_column(conn, "media", "guild_id", "TEXT")

    if fts5_available(conn):
        # feeds_fts is a derived index (rebuilt by the indexer); a legacy
        # table without content_text is dropped and recreated loss-free.
        fts_cols = {
            row[1] for row in conn.execute("PRAGMA table_info(feeds_fts)")
        }
        if fts_cols and "content_text" not in fts_cols:
            conn.execute("DROP TABLE feeds_fts")
